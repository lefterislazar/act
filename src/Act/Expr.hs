...
import EVM.Types hiding (Comp, Prop(..), Expr(..), (.&&), (.>=), (.>), (.<), (./=), (.||))
...
data AExpr (a :: EType) where

  -- identifiers

  -- | Literal words
  Lit            :: {-# UNPACK #-} !W256 -> AExpr EWord
  -- | Variables
  Var            :: Text -> AExpr EWord
  -- | variables introduced during the CSE pass
  GVar           :: GVar a -> AExpr a

  -- bytes

  LitByte        :: {-# UNPACK #-} !Word8 -> AExpr Byte
  IndexWord      :: AExpr EWord -> AExpr EWord -> AExpr Byte
  EqByte         :: AExpr Byte  -> AExpr Byte  -> AExpr EWord

  JoinBytes      :: AExpr Byte -> AExpr Byte -> AExpr Byte -> AExpr Byte
                 -> AExpr Byte -> AExpr Byte -> AExpr Byte -> AExpr Byte
                 -> AExpr Byte -> AExpr Byte -> AExpr Byte -> AExpr Byte
                 -> AExpr Byte -> AExpr Byte -> AExpr Byte -> AExpr Byte
                 -> AExpr Byte -> AExpr Byte -> AExpr Byte -> AExpr Byte
                 -> AExpr Byte -> AExpr Byte -> AExpr Byte -> AExpr Byte
  ...
  Partial        :: [AProp] -> TraceContext -> PartialExec -> AExpr End
  Failure        :: [AProp] -> TraceContext -> EvmError -> AExpr End
  Success        :: [AProp] -> TraceContext -> AExpr Buf -> Map (AExpr EAddr) (AExpr EContract) -> AExpr End
  ITE            :: AExpr EWord -> AExpr End -> AExpr End -> AExpr End

  -- integers

  Add            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Sub            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Mul            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Div            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SDiv           :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Mod            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SMod           :: AExpr EWord -> AExpr EWord -> AExpr EWord
  AddMod         :: AExpr EWord -> AExpr EWord -> AExpr EWord -> AExpr EWord
  MulMod         :: AExpr EWord -> AExpr EWord -> AExpr EWord -> AExpr EWord
  Exp            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SEx            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Min            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Max            :: AExpr EWord -> AExpr EWord -> AExpr EWord

  -- booleans

  LT             :: AExpr EWord -> AExpr EWord -> AExpr EWord
  GT             :: AExpr EWord -> AExpr EWord -> AExpr EWord
  LEq            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  GEq            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SLT            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SGT            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Eq             :: AExpr EWord -> AExpr EWord -> AExpr EWord
  IsZero         :: AExpr EWord -> AExpr EWord

  -- bits

  And            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Or             :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Xor            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  Not            :: AExpr EWord -> AExpr EWord
  SHL            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SHR            :: AExpr EWord -> AExpr EWord -> AExpr EWord
  SAR            :: AExpr EWord -> AExpr EWord -> AExpr EWord

  -- Hashes

  Keccak         :: AExpr Buf -> AExpr EWord
  SHA256         :: AExpr Buf -> AExpr EWord

  -- block context

  Origin         :: AExpr EWord
  BlockHash      :: AExpr EWord -> AExpr EWord
  Coinbase       :: AExpr EWord
  Timestamp      :: AExpr EWord
  BlockNumber    :: AExpr EWord
  PrevRandao     :: AExpr EWord
  GasLimit       :: AExpr EWord
  ChainId        :: AExpr EWord
  BaseFee        :: AExpr EWord
  ...
  Balance        :: AExpr EAddr -> AExpr EWord
  ...
  CodeSize       :: AExpr EAddr         -- address
                 -> AExpr EWord         -- size

  CodeHash       :: AExpr EAddr         -- address
                 -> AExpr EWord         -- size

  -- logs

  LogEntry       :: AExpr EWord         -- address
                 -> AExpr Buf           -- data
                 -> [AExpr EWord]       -- topics
                 -> AExpr Log
  ...
  C ::
    { code     :: ContractCode
    , storage  :: AExpr Storage
    , tStorage :: AExpr Storage
    , balance  :: AExpr EWord
    , nonce    :: Maybe W64
    } -> AExpr EContract
  ...
  SymAddr        :: Text -> AExpr EAddr
  LitAddr        :: Addr -> AExpr EAddr
  WAddr          :: AExpr EAddr -> AExpr EWord

  -- storage

  ConcreteStore  :: (Map W256 W256) -> AExpr Storage
  AbstractStore  :: AExpr EAddr -- which contract is this store for?
                 -> Maybe W256 -- which logical store does this refer to? (e.g. solidity mappings / arrays)
                 -> AExpr Storage

  SLoad          :: AExpr EWord         -- key
                 -> AExpr Storage       -- storage
                 -> AExpr EWord         -- result

  SStore         :: AExpr EWord         -- key
                 -> AExpr EWord         -- value
                 -> AExpr Storage       -- old storage
                 -> AExpr Storage       -- new storae

  -- buffers

  ConcreteBuf    :: ByteString -> AExpr Buf
  AbstractBuf    :: Text -> AExpr Buf

  ReadWord       :: AExpr EWord         -- index
                 -> AExpr Buf           -- src
                 -> AExpr EWord

  ReadByte       :: AExpr EWord         -- index
                 -> AExpr Buf           -- src
                 -> AExpr Byte

  WriteWord      :: AExpr EWord         -- dst offset
                 -> AExpr EWord         -- value
                 -> AExpr Buf           -- prev
                 -> AExpr Buf

  WriteByte      :: AExpr EWord         -- dst offset
                 -> AExpr Byte          -- value
                 -> AExpr Buf           -- prev
                 -> AExpr Buf

  CopySlice      :: AExpr EWord         -- src offset
                 -> AExpr EWord         -- dst offset
                 -> AExpr EWord         -- size
                 -> AExpr Buf           -- src
                 -> AExpr Buf           -- dst
                 -> AExpr Buf

  BufLength      :: AExpr Buf -> AExpr EWord

deriving instance Show (AExpr a)
deriving instance Eq (AExpr a)
deriving instance Ord (AExpr a)

data AProp where
  PEq :: forall a . Typeable a => AExpr a -> AExpr a -> AProp
  PLT :: AExpr EWord -> AExpr EWord -> AProp
  PGT :: AExpr EWord -> AExpr EWord -> AProp
  PGEq :: AExpr EWord -> AExpr EWord -> AProp
  PLEq :: AExpr EWord -> AExpr EWord -> AProp
  PNeg :: AProp -> AProp
  PAnd :: AProp -> AProp -> AProp
  POr :: AProp -> AProp -> AProp
  PImpl :: AProp -> AProp -> AProp
  PBool :: Bool -> AProp
deriving instance (Show AProp)
...
infix 4 .==, ./=
(.==) :: (Typeable a) => AExpr a -> AExpr a -> AProp
x .== y = PEq x y
(./=) :: (Typeable a) => AExpr a -> AExpr a -> AProp
x ./= y = PNeg (PEq x y)
...
instance Eq AProp where
  PBool a == PBool b = a == b
  PEq (a :: AExpr x) (b :: AExpr x) == PEq (c :: AExpr y) (d :: AExpr y)
    = case eqT @x @y of
       Just Refl -> a == c && b == d
       Nothing -> False
...
instance Ord AProp where
  compare (PBool a) (PBool b) = compare a b
  compare (PEq (a :: AExpr x) (b :: AExpr x)) (PEq (c :: AExpr y) (d :: AExpr y)) =
    case eqT @x @y of
...
--    T.putStrLn $ indent 3 (T.pack "\x1b[33m[WARNING]\x1b[0m: hevm was only able to partially explore ")
--                <> T.pack call <> (T.pack " due to the following issue(s):")
--    T.putStr . T.unlines . fmap (indent 5 . (T.pack "- " <>)) . fmap formatPartial . getPartials $ flattened
--
...
--      let branchesA = rewriteFresh (T.pack "A-") branchesAorig
...
equivalenceCheck'
  :: forall m . App m
  => SolverGroup -> [Types.Expr End] -> [AExpr End] -> Bool -> m EqIssues
equivalenceCheck' solvers branchesA branchesB create = do
  ...
  where
    ...
    checkAll :: (App m, MonadUnliftIO m) => [(Set AProp, String)] -> Int -> m [(EquivResult, String)]
    ...
    distinct :: App m => Types.Expr End -> AExpr End -> m (Maybe (Set AProp, String), EqIssues)
    distinct aEnd bEnd = do
      ...
      where
        collectReqs (Just (reqToDiff, meaning)) = Just (Set.fromList $ Expr.simplifyProps (reqToDiff : Types.extractProps aEnd <> extractPropsA bEnd), meaning)
    ...
    resultsDiffer :: App m => Types.Expr End -> AExpr End -> m (Maybe (AProp, String), EqIssues)
    resultsDiffer aEnd bEnd = do
      let deployText :: String = if create then "Undeployed contracts. " else "Deployed contracts. "
      case (aEnd, bEnd) of
        (Types.Success aProps _ aOut aState, Success bProps _ bOut bState) ->
        ...
        where
          ...
          differentError :: EvmError ->EvmError -> AProp
    ...
    deployedCodeDiffer :: Types.Expr Buf -> AExpr Buf -> [Types.Prop] -> [AProp] -> m EqIssues
    ...
    statesDiffer :: Map (Types.Expr EAddr) (Types.Expr EContract) -> Map (AExpr EAddr) (AExpr EContract) -> AProp
    ...
    contractsDiffer :: Types.Expr EContract -> AExpr EContract -> AProp
...
--    Just (Types.Comp _) -> internalError "CompressedBuf returned from collapse"
--
...
--        forceFlattened buf@(Types.Comp _) =  case SMT.collapse buf of
